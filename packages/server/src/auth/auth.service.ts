...
export class AuthService {
  ...
  async signinLocal(dto: AuthDto) {
    const user = await this.prisma[dto.role].findFirst({
      where: {
        email: dto.email,
      },
    });
    ...
    const tokens = await this.getTokens(user.id, dto.role);
    await this.updateRtHash(user.id, tokens.refresh_token, dto.role);
  ...
  async forgotPassToken(email: string, role: string) {
    const user = await this.prisma[role].findFirst({
      where: {
        email,
      },
  ...
  async verifyForgotPassToken(token: string, password: string, role: string) {
